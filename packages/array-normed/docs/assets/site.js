...
// Filter UI
let tocElements = document.getElementById('toc').getElementsByTagName('li');
...
document.getElementById('filter-input').addEventListener('keyup', function (e) {
  ...
  if (!value.match(/^\s*$/)) {
    match = function (element) {
      let html = element.firstChild.innerHTML;
...
let split_left = document.querySelector('#split-left');
let split_right = document.querySelector('#split-right');
let split_parent = split_left.parentNode;
let cw_with_sb = split_left.clientWidth;
split_left.style.overflow = 'hidden';
let cw_without_sb = split_left.clientWidth;
